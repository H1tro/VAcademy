...
import { getInformatikaFilesList, getInformatikaFileBlob } from "@/lib/informatika-blob"
...
export async function GET(req: Request) {
  try {
    ...
    if (!file) {
      try {
        const items = await getInformatikaFilesList()
    ...
    const buffer = await getInformatikaFileBlob(file)
